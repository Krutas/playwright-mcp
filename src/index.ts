...
let extTabId: number | null = null;
let cmdCallbacks = new Map<string, (result: any) => void>();
let cmdCounter = 0;
...
function sendToExtension(data: any): Promise<any> {
...
async function handleToolCall(name: string, args: any) {
  ...
  if (name === "extension_evaluate") {
    ...
    if (result.error) return textContent("Error: " + result.error);
  ...
  if (name === "extension_get_tabs") {
    ...
    if (result.tabs) {
      const list = result.tabs.map((t: any) => "[" + t.id + "] " + (t.title || "untitled") + " - " + (t.url || "about:blank") + (t.active ? " [ACTIVE]" : ""));
  ...
  if (name === "extension_click") {
    ...
    if (docResult.error) return textContent("Error: " + docResult.error);
    const rootNodeId = docResult.result?.result?.root?.nodeId;
    if (!rootNodeId) return textContent("Could not get document root");
    const queryResult = await sendToExtension({
      type: "command",
      method: "DOM.querySelector",
      params: { nodeId: rootNodeId, selector }
    });
    if (queryResult.error) return textContent("Error: " + queryResult.error);
    const nodeId = queryResult.result?.result?.nodeId;
    if (!nodeId) return textContent("Element not found: " + selector);
